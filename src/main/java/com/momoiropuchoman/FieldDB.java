...
class FieldDB extends DB {
	...
	Set<Chara> getCharas(String fieldName) {
		...
		try {
			...
			while(resultSet.next()) {
				charas.add(new Chara(new Position(resultSet.getInt("x"), resultSet.getInt("y")), resultSet.getInt("imageNo"), resultSet.getString("name")));
			} 
